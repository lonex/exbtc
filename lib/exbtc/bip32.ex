defmodule Exbtc.BIP32 do 
  ...
  @spec raw_bip32_ckd({charlist, Integer, charlist, Integer, charlist, charlist}, Integer) :: { charlist, charlist }
  def raw_bip32_ckd({vbytes, depth, fingerprint, _oldi, chaincode, key}, i) do
    {priv, pub} = if vbytes in @private do
      {key, C.privkey_to_pubkey(key)}
    else
      {nil, key}
    end

    cap_i = :binary.bin_to_list(if i >= U.power(2, 31) do
      if vbytes in @public do
        raise "Cannot do private derivation on public key"
      else
        :crypto.hmac(:sha512, chaincode, [0] ++ String.slice(priv, 0, 31) ++ C.encode(i, 256, 4))
      end
    else
      :crypto.hmac(:sha512, chaincode, pub ++ C.encode(i, 256, 4))
    end)

    {newkey, fingerprint} = cond do
      vbytes in @private ->
        { C.add_privkeys(Enum.slice(cap_i, 0..31) ++ [1], priv), Enum.slice(C.bin_hash160(C.privkey_to_pubkey(key)), 0..3) }
      vbytes in @public ->
  ...
  def bip32_serialize({vbytes, depth, fingerprint, i, chaincode, key}) do
    ...
    keydata = if vbytes in @private do
  ...
  @spec bip32_deserialize(String.t) :: { charlist, Integer, charlist, Integer, charlist, charlist }
  ...
  def raw_bip32_privtopub({vbytes, depth, fingerprint, i, chaincode, key}) do
    new_vbytes = if vbytes == @mainnet_private do
      @mainnet_public
    else
      @testnet_public
    end
  ...
  def bip32_master_key(seed, vbytes \\ @mainnet_private) do
    cap_i = :binary.bin_to_list(:crypto.hmac(:sha512, C.from_string_to_bytes("Bitcoin seed"), C.from_string_to_bytes(seed)))
    {vbytes, 0, U.replicate(4, 0), 0, Enum.slice(cap_i, 32..length(cap_i)), Enum.slice(cap_i, 0..31) ++ [1]} |> bip32_serialize
  end
